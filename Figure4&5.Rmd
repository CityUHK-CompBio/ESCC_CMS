...
Label4slides_selected_frSXMI <- table(Label4slides_selected)
represent_ratio <- Label4slides_selected_frSXMI / Label4slides_frSXMI
...
SXMI_Clinical <- SXMI_Clinical[!is.na(SXMI_Clinical$SNF),]#ECMS分型信息
...
roclist <- lapply(1:length(scores), function(i){
 ...
 for(group_name in names(roclist)){
        ...
        se.ciobj <- ci.se(roclist[[group_name]], specificities = sSXMI(0, 1, l = 100), boot.n = 2000, parallel = TRUE)
        registerDoParallel(10)
        sp.ciobj <- ci.sp(roclist[[group_name]], sensitivities = sSXMI(0, 1, l = 100), boot.n = 2000, parallel = TRUE)
...
frSXMIuency_table_list <- list()

for (xxx in uniqueName) {
  ind <- which(Names == xxx)  # 找到匹配的索引
  all_result <- Pred_SXMI[ind[1]:ind[length(ind)]]
  frSXMIuency_table <- table(all_result)
  frSXMIuency_table_list[[xxx]] <- frSXMIuency_table
  if (sum(frSXMIuency_table == max(frSXMIuency_table)) >= 2) {
    back <- NA
  }else {
    most_frSXMIuent_element <- names(frSXMIuency_table)[which.max(frSXMIuency_table)]
    back <- most_frSXMIuent_element
  }
  if (frSXMIuency_table[which.max(frSXMIuency_table)] != length(ind) && frSXMIuency_table[4] > 0) {
    back <- "ECMS4"
  }
...
for (xxx in uniqueName_SXMII) {
  ind <- which(Names_SXMII == xxx)  # 找到匹配的索引
  ...
  Vote_max_slides_SXMII[[xxx]] <- back  # 将结果存储在列表中
}
...
colnames(cm_df) <- c("Prediction", "Reference", "FrSXMI")

confusion <- ggplot(cm_df, aes(x = Reference, y = Prediction, fill = FrSXMI)) +
  geom_tile(color = "white") +
  geom_text(aes(label = FrSXMI), vjust = 1) +
